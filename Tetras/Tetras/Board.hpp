...
#ifndef __Tetras__Board__
#define __Tetras__Board__
...
public:
    ...
    Board(int columns=DEFAULT_BOARD_COLUMNS
          ,int line=DEFAULT_BOARD_LINE
          ,int cell_x=DEFAULT_CELL_X
          ,int cell_y=DEFAULT_CELL_Y);
    ~Board();
...
private:
    ...
    void flood(int grid_x
               ,int grid_y
               ,int piece_x
               ,int piece_y
               ,Piece::TetriminoTypes type
               ,int rotation
               ,bool visited[][Piece::MATRIX_SIZE]
               ,int value);
...
#endif /* defined(__Tetras__Board__) */
